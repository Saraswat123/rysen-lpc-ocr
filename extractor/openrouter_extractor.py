"""
OpenRouter backend — free vision models, works in India.
Uses OpenAI-compatible API with PDF→images conversion.

Get free API key: https://openrouter.ai/keys
Free models: meta-llama/llama-4-scout:free, google/gemma-3-27b-it:free
"""
import base64
import json
import os
...
class OpenRouterLPCExtractor:
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        key = api_key or os.environ.get("OPENROUTER_API_KEY")
        if not key:
            raise ValueError("OPENROUTER_API_KEY not set. Get free key at openrouter.ai/keys")
        self.client = OpenAI(
            api_key=key,
            base_url="https://openrouter.ai/api/v1",
            default_headers={
                "HTTP-Referer": "https://aitsgroup.com",
                "X-Title": "RYSEN LPC OCR",
            },
        )
    ...
    def extract_pdf(self, pdf_path: str, retry: int = 3) -> LPCRecord:
        ...
        print(f"  {len(images_b64)} page(s) → sending to OpenRouter ({self.model}) ...")
        ...
        for attempt in range(retry + 1):
            try:
                ...
                if attempt < retry:
                    ...
                    raise RuntimeError(f"OpenRouter extraction failed for {pdf_path}: {e}") from e
